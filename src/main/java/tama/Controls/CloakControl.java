...
public class CloakControl extends Control<ControlDataBool> {
    ...
    public InteractionResult onUse(Player player, InteractionHand hand, ITardisLevel level) {
        ...
        if(level.getLevel().isClientSide) return InteractionResult.PASS;

        if(ServerLifecycleHooks.getCurrentServer().getLevel(level.getDestination().getLevel()).getBlockEntity(level.getDestination().getPos()) instanceof ExteriorTile exteriorTile) {
            player.sendSystemMessage(Component.translatable("control.aseoha.cloak", !((IHelpWithMatterStateHandlers) exteriorTile.getMatterStateHandler()).Aseoha$IsCloaked() ? "On" : "Off"));
            ((IHelpWithMatterStateHandlers) exteriorTile.getMatterStateHandler()).Aseoha$SetCloaked(!((IHelpWithMatterStateHandlers) exteriorTile.getMatterStateHandler()).Aseoha$IsCloaked());
        }
