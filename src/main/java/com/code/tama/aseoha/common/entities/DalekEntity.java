/* (C) TAMA Studios 2026 */
package com.code.tama.aseoha.common.entities;
...
public class DalekEntity extends Monster {
	protected static final int ATTACK_TIME = 80;
	private static final EntityDataAccessor<Boolean> DATA_ID_MOVING = SynchedEntityData.defineId(Guardian.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> DATA_ID_ATTACK_TARGET = SynchedEntityData.defineId(Guardian.class,
	...
	public DalekEntity(EntityType<? extends DalekEntity> p_32810_, Level p_32811_) {
		...
		this.moveControl = new DalekEntity.GuardianMoveControl(this);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(4, new DalekEntity.GuardianAttackGoal(this));
		...
		this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, true, false,
				new DalekEntity.GuardianAttackSelector(this)));
	}
		...
		if (this.isAlive()) {
			if (this.level().isClientSide) {
				...
				if (!this.isInWater()) {
					...
					this.clientSideTouchedGround = vec3.y < 0.0D
				...
				if (this.isMoving() && this.isInWater()) {
					...
					for (int i = 0; i < 2; ++i) {
						this.level().addParticle(ParticleTypes.BUBBLE, this.getRandomX(0.5D) - vec31.x * 1.5D,
								this.getRandomY() - vec31.y * 1.5D, this.getRandomZ(0.5D) - vec31.z * 1.5D, 0.0D, 0.0D,
								0.0D);
					}
				}
				...
				if (this.hasActiveAttackTarget()) {
					...
					if (livingentity != null) {
						...
						while (d4 < d3) {
							...
							this.level().addParticle(ParticleTypes.BUBBLE, this.getX() + d0 * d4,
									this.getEyeY() + d1 * d4, this.getZ() + d2 * d4, 0.0D, 0.0D, 0.0D);
						}
					}
				}
			}
			...
			if (this.isInWaterOrBubble()) {
				...
				this.setDeltaMovement(
						this.getDeltaMovement().add((double) ((this.random.nextFloat() * 2.0F - 1.0F) * 0.4F), 0.5D,
	...
	public static boolean checkGuardianSpawnRules(EntityType<? extends Guardian> p_218991_, LevelAccessor p_218992_,
			MobSpawnType p_218993_, BlockPos p_218994_, RandomSource p_218995_) {
	...
	public boolean hurt(DamageSource p_32820_, float p_32821_) {
		if (this.level().isClientSide) {
			...
			if (!this.isMoving() && !p_32820_.is(DamageTypeTags.AVOIDS_GUARDIAN_THORNS)
	...
	static class GuardianAttackGoal extends Goal {
		...
		private final boolean elder; // TODO: Rename to "Emperor" for dalek-ness

		public GuardianAttackGoal(DalekEntity p_32871_) {
		...
		public boolean canContinueToUse() {
			return super.canContinueToUse() && (this.elder || this.guardian.getTarget() != null
					&& this.guardian.distanceToSqr(this.guardian.getTarget()) > 9.0D);
		}
			...
			if (livingentity != null) {
				...
				if (!this.guardian.hasLineOfSight(livingentity)) {
					...
					if (this.attackTime == 0) {
						...
						float f = (this.guardian.level().getDifficulty() == Difficulty.HARD)
								? Float.MAX_VALUE
								: this.elder ? 20.0f : 10.0f;

						livingentity.hurt(this.guardian.damageSources().indirectMagic(this.guardian, this.guardian), f);
						livingentity.hurt(this.guardian.damageSources().mobAttack(this.guardian),
	...
	static class GuardianAttackSelector implements Predicate<LivingEntity> {
		...
		public GuardianAttackSelector(DalekEntity p_32879_) {
		...
		public boolean test(@Nullable LivingEntity p_32881_) {
			return (p_32881_ instanceof Player || p_32881_ instanceof Squid || p_32881_ instanceof Axolotl)
					&& p_32881_.distanceToSqr(this.guardian) > 9.0D;
		}
	}
