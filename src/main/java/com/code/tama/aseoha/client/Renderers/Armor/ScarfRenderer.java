...
public class ScarfRenderer<T extends ScarfModel> {
	...
	public ModelPart makeArmorParts(EquipmentSlot slot) {
		return new ModelPart(Collections.emptyList(),
				Map.of("head", EMPTY_PART, "hat", EMPTY_PART, "body",
						slot == EquipmentSlot.CHEST ? checkNonNull(model.Body) : EMPTY_PART, "right_arm", EMPTY_PART,
						"left_arm", EMPTY_PART, "right_leg", EMPTY_PART, "left_leg", EMPTY_PART));
	}
