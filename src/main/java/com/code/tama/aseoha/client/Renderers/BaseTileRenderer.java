/* (C) TAMA Studios 2025 */
package com.code.tama.aseoha.client.Renderers;
...
public class BaseTileRenderer<T extends BlockEntity, M extends EntityModel<Entity> & IAnimatableTileModel<T>>
	...
	/**
	 * @param model
	 *            the model that will get rendered
	 * @param texture
	 *            the texture to render the layer with
	 */
	...
	public void render(@NotNull T t, float v1, PoseStack poseStack, MultiBufferSource buffer, int light, int v2) {
		...
		long animTicks = Capabilities.getCap(Capabilities.TARDIS, t.getLevel()).map(ITardisLevel::getAnimationTicks)
				.orElse(t.getLevel().getGameTime());

		this.model.setupAnimations(t, animTicks + v1);
		this.model.renderToBuffer(poseStack, buffer.getBuffer(RenderType.entityTranslucent(texture)), light, v2, 1, 1,
				1, 1);
		poseStack.popPose();
	}
}
