...
public class ManualScreen extends Screen {
	...
	private static final MutableComponent NEXT_PAGE = Component.translatable("tooltip.tardis.manual.next_page");
	private static final MutableComponent PREV_PAGE = Component.translatable("tooltip.tardis.manual.previous_page");
	private static final MutableComponent NEXT_CHAPTER = Component.translatable("tooltip.tardis.manual.next_chapter");
	private static final MutableComponent PREV_CHAPTER = Component
			.translatable("tooltip.tardis.manual.previous_chapter");
	private static final MutableComponent LAST_PAGE = Component.translatable("tooltip.tardis.manual.last_page");
	private static final MutableComponent FIRST_PAGE = Component.translatable("tooltip.tardis.manual.first_page");
	...
	public ManualScreen(ItemStack stack) {
		this(Component.literal("Manual"), aseoha.MODID);
		if (stack.hasTag()) {
	...
	protected void init() {
		...
		this.addRenderableWidget(
				this.pageForward = new ChangePageButton(this.width / 2 + 95, this.height / 2 + 70, true, (button) -> {
					this.turnPage(true);
				}, true));
		this.addRenderableWidget(
				this.pageBack = new ChangePageButton(this.width / 2 - 120, this.height / 2 + 70, false, (button) -> {
					this.turnPage(false);
				}, true));
		this.addRenderableWidget(this.chapterForward = new ChangeChapterButton(this.width / 2 + 25,
				this.height / 2 + 70, true, (button) -> {
		...
		if (maxChapters > 0) {
			...
			if (numPages > 0) {
				...
				for (int i = 0; i < numPages; ++i) {
					...
					if (i < chapter.getPages().size() && !(chapter.getPages().get(i) instanceof TOCPage)) {
						chapter.insertPage(i, new TOCPage(this, startIndex, endIndex));
					} else if (i >= chapter.getPages().size()) {
						chapter.insertPage(i, new TOCPage(this, startIndex, endIndex));
					}
				}
			}
	...
	public void turnToTableOfContents(boolean forward) {
		...
		if (forward) {
			...
			Chapter lastChapter = this.chapters.get(maxChapter);
			if (lastChapter != null && lastChapter.getPages() != null && !lastChapter.getPages().isEmpty()) {
				this.chapterIndex = maxChapter;
				this.pageIndex = lastChapter.getPages().size() - 1;
			} else {
				this.chapterIndex = 0;
				this.pageIndex = 0;
			}
	...
	public Chapter getChapter() {
		return this.chapterIndex < this.chapters.size() ? this.chapters.get(this.chapterIndex) : null;
	}
	...
	private void read() {
		...
		try {
			...
			aseoha.LOGGER.error("Could not find Manual resources for locale: {}, reverting to contents for locale {}",
