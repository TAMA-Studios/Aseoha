...
public class BOTIUtils {
	...
	// }
	//
	// public static void GatherChunkData(AbstractPortalTile portalTile, Level
	// level, int chunks) {
	// if (!TTSConfig.ServerConfig.BOTI_ENABLED.get())
	// return;
	// BlockPos targetPos = portalTile.getTargetPos();
	// new ChunkGatheringThread(chunks, (ServerLevel) level, portalTile,
	// targetPos).start();
	// }
	//
	...
	// {
	// pose.pushPose();
	// portal.getFBOContainer().Render(pose, (stack, botiSource) ->
	// StencilUtils.drawFrame(stack, 1, 2),
	// (stack, buff) -> {
	...
	//
	// if (currentTime - portal.lastUpdateTime >= 1200) { // update model every 1200
	// ticks, or a minute TODO: make
	...
	// }
	//
	// if (portal.MODEL_VBO == null) { // It'll be null the first time it's
	// accessed, forcing a build
	// portal.MODEL_VBO = BOTIUtils.buildModelVBO(portal.containers, portal); //
	// Build VBO so it's not null
	...
	// portal.MODEL_VBO.drawWithShader(pose.last().pose(),
	...
	// AbstractPortalTile tile) {
	// Minecraft mc = Minecraft.getInstance();
	//
	// int ChunksToRender = 8;
	//
	// BufferBuilder buffer = new BufferBuilder((int) (ChunksToRender * Math.pow(16,
	// 3)));
	...
	//
	// Map<BlockPos, BotiChunkContainer> chunkMap =
	// getMapFromContainerList(containers);
	//
	// chunkMap.forEach((pos, container) -> {
	// BlockColors colors = mc.getBlockColors();
	// int color = colors.getColor(container.getState(),
	// Minecraft.getInstance().level, container.getPos(), 0);
	//
	...
	// buffer.vertex(v.x, v.y, v.z).color(v.r, v.g, v.b, v.a).uv(v.u,
	// v.v).uv2(container.getLight())
	// .endVertex();
	// }
	// }
	// }
	//
	// for (BakedQuad quad : getModelFromBlock(container.getState(), pos, rand,
	// chunkMap)) {
	...
	//
	// buffer.putBulkData(stack.last(), quad, rLit, gLit, bLit, 1.0F,
	// container.getLight(),
	...
	// }
	//
	// public static Map<BlockPos, BotiChunkContainer>
	// getMapFromContainerList(List<BotiChunkContainer> list) {
	...
	// }
	//
	// public static List<BakedQuad> getModelFromBlock(BlockState state, BlockPos
	// pos, RandomSource rand,
	...
	// }
	//
	// public static boolean isSideVisibleFrom(BlockPos from, BlockPos to, Direction
	// side) {
	// // Get center points for both blocks
	// Vec3 fromCenter = new Vec3(from.getX() + 0.5, from.getY() + 0.5, from.getZ()
	// + 0.5);
	...
	// Vec3 faceNormal = new Vec3(side.getStepX(), side.getStepY(),
	// side.getStepZ());
	//
	...
	// }
	//
	// public static boolean shouldRenderFace(BlockState state, BlockState neighbor,
	...
	// Block.BlockStatePairKey block$blockstatepairkey = new
	// Block.BlockStatePairKey(state, neighbor, dir);
	// Object2ByteLinkedOpenHashMap<Block.BlockStatePairKey>
	// object2bytelinkedopenhashmap = BlockAccessor
	// .getOcclusionCache().get();
	// byte b0 =
	// object2bytelinkedopenhashmap.getAndMoveToFirst(block$blockstatepairkey);
	// if (b0 != 127) {
	// return b0 != 0;
	// } else {
	...
	// return true;
	// } else {
	// VoxelShape voxelshape1 = neighbor.getFaceOcclusionShape(level, secondPos,
	// dir.getOpposite());
	// boolean flag = Shapes.joinIsNotEmpty(voxelshape, voxelshape1,
	// BooleanOp.ONLY_FIRST);
	// if (object2bytelinkedopenhashmap.size() == 2048) {
	// object2bytelinkedopenhashmap.removeLastByte();
	// }
	//
	// object2bytelinkedopenhashmap.putAndMoveToFirst(block$blockstatepairkey,
	// (byte) (flag ? 1 : 0));
	// return flag;
	// }
	// }
	...
	// Networking.INSTANCE
	// .sendToServer(new PortalChunkRequestPacketC2S(tileEntity.getBlockPos(),
	// tileEntity.getTargetLevel(),
	// tileEntity.getTargetPos(),
	// TTSConfig.ClientConfig.BOTI_RENDER_DISTANCE.get()));
	//
